...
export class MainScene extends g.Scene {
	...
	constructor(param: g.SceneParameterObject) {
		param.assetIds = [
			"img_numbers_n", "img_numbers_n_red", "img_numbers_n_blue", "title", "start", "finish", "score", "time",
			"ball","coin","item","floor","effect","pentagon","shape",
			"config", "volume", "test", "glyph72", "number_k", "number_b", "number_y", "number_p",
			
			"se_start", "se_timeup", "bgm","clear1","se_clear","biri","miss","se_hit"];
		super(param);
		...
		this.loaded.add(() => {
			...
			//タイトル
			const sprTitle = new g.Sprite({ scene: this, src: this.assets["title"]});
			...
			const numFontBlue = new g.BitmapFont({
				src: this.assets["img_numbers_n_blue"],
				map: glyph.map,
				defaultGlyphWidth: glyph.width,
				defaultGlyphHeight: glyph.height,
				missingGlyph: glyph.missingGlyph
			});
			this.numFontBlue = numFontBlue;
			...
			//スコア
			//uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 420, y: 5, height: 32 }));
			let score = 0;
			...
			//タイム
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["time"], x: 0, y: 5 }));
			const labelTime = new g.Label({ scene: this, font: numFont, fontSize: 32, text: "70", x: 40, y: 8 });
